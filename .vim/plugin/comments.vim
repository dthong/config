...
noremap  <silent> <D-/> :call RangeCommentLine()<CR>
" key-mappings for range comment lines in visual <Shift-V> mode
vnoremap <silent> <D-/> :call RangeCommentLine()<CR>
inoremap <silent> <D-/> <C-o>:call RangeCommentLine()<CR>

" key-mappings for un-comment line in normal mode
noremap  <silent> <D-\> :call UnCommentLine()<CR>
...
function! CommentLine()
  ...
  elseif file_name =~ '\.html$' || file_name =~ '\.htm$' || file_name =~ '\.xml$' || file_name =~ '\.xhtml$' || file_name =~ '\.erb$'
    ...
    elseif stridx( getline("."), "\<!--" ) == -1 && stridx( getline("."), "--\>" ) != -1
       execute ":silent! normal ^i\<\!--\<ESC>==\<down>^"
    elseif stridx( getline("."), "\<!--" ) == -1 && stridx( getline("."), "--\>" ) == -1
       execute ":silent! normal ^i\<\!-- \<ESC>lx$a --\>\<ESC>==\<down>^"
    endif
  " for .vim files use "
